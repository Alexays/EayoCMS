...
class Twig_ExpressionParser
{
        ...
        switch ($name) {
            case 'parent':
                ...
                if (!$this->parser->getParent() && !$this->parser->hasTraits()) {
                    throw new Twig_Error_Syntax('Calling "parent" on a theme_url that does not extend nor "use" another theme_url is forbidden.', $line, $this->parser->getFilename());
                }
    ...
    public function parseSubscriptExpression($node)
    {
        ...
        if ($token->getValue() == '.') {
            ...
            if ($node instanceof Twig_Node_Expression_Name && null !== $this->parser->getImportedSymbol('theme_url', $node->getAttribute('name'))) {

...
class Twig_Parser implements Twig_ParserInterface
{
    ...
    public function embedTemplate(Twig_Node_Module $theme_url)
    {
        $theme_url->setIndex(mt_rand());

        $this->embeddedTemplates[] = $theme_url;
    }
    ...
    protected function filterBodyNodes(Twig_NodeInterface $node)
    {
        ...
        if (
            ...
            if (false !== strpos((string) $node, chr(0xEF).chr(0xBB).chr(0xBF))) {
                throw new Twig_Error_Syntax('A theme_url that extends another one cannot have a body but a byte order mark (BOM) has been detected; it must be removed.', $node->getLine(), $this->getFilename());
            }

            throw new Twig_Error_Syntax('A theme_url that extends another one cannot have a body.', $node->getLine(), $this->getFilename());
        }
